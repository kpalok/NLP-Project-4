...
def fuzzyvariation(list,titles):
    ...
    exporttoexcel(elements,header,titles[0],'fuzzy') 
...
def wupsimilarity(listwup,titles):
    ...
    header=terminaltable(elements,titles)
    exporttoexcel(elements,header,titles[0],'wup') 



def exporttoexcel(elements,header,title,type):

    content=tabulate(elements, headers=header)
    filenamefuzzy="output table of fuzzy variation of %s book.csv" %title
    filenamewup="output table of WUP variation of %s book.csv" %title
    if type == 'fuzzy':
        text_file=open(filenamefuzzy,"w")
    else:
        text_file=open(filenamewup,"w")
...
###################################### the prophet book analysis ####################################
    f = open(TheProphet +".txt")
    Book = f.read()
    distlist= Task1.poetryanalysis(Book)
    

    list= getcommonwords(distlist)                #list of most common 30 words
    titles= gettitles(Book,'The Prophet') 
    fuzzyvariation(list,titles)


    distlistwup= Task1.poetryanalysisWOstemming(Book) #tokenizing the book without stemming because it affects the accuracy of word net
...
###################################### Childs garden book analysis ####################################
    ...
    list= getcommonwords(distlist)                #list of most common 30 words
    titles= gettitles(Book,'Childs Garden') 
    fuzzyvariation(list,titles)
